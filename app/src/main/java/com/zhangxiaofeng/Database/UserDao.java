...
public class UserDao {
    ...
    public int add(UserData userInfo) {
        ...
        db.execSQL("insert into user (_id,username,password) values (?,?,?)",
                new Object[]{
                        userInfo.get_id(),
    ...
    public List<UserData> getAll() {
        ...
        db = helper.getWritableDatabase(); // 初始化 SQLiteDatabase 对象
        Cursor cursor = db.rawQuery("SELECT * FROM user", null); // 查询全部数据
    ...
    public int getMaxId() {
        db = helper.getWritableDatabase();// 初始化 SQLiteDatabase 对象
        Cursor cursor = db.rawQuery("select max(_id) from user", null);// 获取便签信息表中的最大编号
    ...
    public UserData find(int id) {
        ...
        Cursor cursor = db.rawQuery(
                "select _id,flag from user where _id = ?",
    ...
    public void update(UserData tb_flag) {
        db = helper.getWritableDatabase();// 初始化 SQLiteDatabase 对象
        db.execSQL("update user set username = ?, password = ? where _id = ?",
    ...
    public long getCount() {
        db = helper.getWritableDatabase();// 初始化 SQLiteDatabase 对象
        Cursor cursor = db.rawQuery("select count(_id) from user", null);// 获取便签信息的记录数
